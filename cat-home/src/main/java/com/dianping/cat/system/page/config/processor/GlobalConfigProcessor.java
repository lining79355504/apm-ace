...
public class GlobalConfigProcessor {
    ...
    public void process(Action action, Payload payload, Model model) {
        switch (action) {
            ...
            case SAMPLE_CONFIG_UPDATE:
                ...
                model.setContent(m_configHtmlParser.parse(m_sampleConfigManager.getSampleConfig().toString()));
                break;
    ...
    private boolean adminCheck(Payload payload) {
        if ("admin-business".equals(payload.getLoginUser())) {
            return true;
        }
        return false;
    }
